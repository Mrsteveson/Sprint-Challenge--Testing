...
1. In Jest, what are the differences between `describe()` and `it()` globals, and what are good uses for them?
    ...
    Using describe() allows you to break up our test suite's into easily read bricks/components that test individual aspects of the application.
    Using it() allows individual tests to be ran in order to prevent unforeseen bugs.
